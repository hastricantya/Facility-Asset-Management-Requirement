...
import { ChevronsUpDown, ChevronLeft, ChevronRight, Eye, MoreHorizontal, Home, MapPin } from 'lucide-react';
        ...
        <table className="w-full min-w-[900px] text-left border-collapse">
          <thead>
            <tr className="bg-[#F2F2F2] border-b border-gray-200">
              ...
              <th className="p-5 w-48 text-[10px] font-black text-black uppercase tracking-[0.15em]">Room Preference</th>
              ...
              <tr key={item.id} className="bg-white hover:bg-[#FDFDFD] transition-all group cursor-pointer" onClick={() => onView?.(item)}>
                ...
                <td className="p-5">
                  <div className="space-y-1">
                    <div className="flex items-center gap-1.5 text-black font-black text-[11px] uppercase">
                      <MapPin size={10} className="text-gray-400" /> {item.preferredFloor}
                    </div>
                    <div className="flex items-center gap-1.5 text-gray-400 font-bold text-[10px] uppercase">
                      <Home size={10} className="text-gray-300" /> {item.preferredRoomType}
                    </div>
                  </div>
                </td>
                <td className="p-5">
